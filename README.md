...
- **Closed type:** `type Decision = ApproveDose(DoseBand) | RejectDose` where `DoseBand` is closed
- **Sink / capability:** `grant approveDose irreversible confidence 80` → `commit { approveDose(d) }`
- **Consequence axes:** Trust, Confidence, Reversibility

## Run the demo

```sh
examples/apps/dosage-gate/demo.sh
```
...
- `dosage-gate_safe.wave` — the correct design.
- `dosage-gate_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
